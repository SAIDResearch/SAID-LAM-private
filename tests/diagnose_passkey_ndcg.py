#!/usr/bin/env python3
"""
LEMBPasskeyRetrieval NDCG@1 diagnostic — aligned with MTEB retrieval scoring.
...
def main():
    parser = argparse.ArgumentParser(description="Diagnose LEMBPasskeyRetrieval NDCG@1 per split")
    ...
    print("Loading LEMBPasskeyRetrieval task...")
    tasks = list(mteb.get_tasks(tasks=["LEMBPasskeyRetrieval"], languages=["eng"]))
    if not tasks:
        print("ERROR: No LEMBPasskeyRetrieval task found")
        sys.exit(1)
    task = tasks[0]
    task.load_data()
